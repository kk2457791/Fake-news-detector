...
def load_dataset():
    ...
    try:
        ...
        print("Dataset files not found. Please run model_training.py first to download the dataset.")
        return None
...
def analyze_dataset(df):
    ...
    print(f"Real news: {avg_length[0]:.2f} words")
    print(f"Fake news: {avg_length[1]:.2f} words")
...
def extract_frequent_words(df):
    ...
    plt.subplot(1, 2, 1)
    plt.bar(top_real_words.keys(), top_real_words.values())
    plt.title('Most Frequent Words in Real News')
    plt.xticks(rotation=90)
    
    # Plot word frequency for fake news
    plt.subplot(1, 2, 2)
    plt.bar(top_fake_words.keys(), top_fake_words.values())
...
def engineer_features(df):
    ...
    df_sample['flesch_reading_ease'] = df_sample['text'].progress_apply(
        lambda x: textstat.flesch_reading_ease(str(x)) if isinstance(x, str) else 0
    )
    df_sample['flesch_kincaid_grade'] = df_sample['text'].progress_apply(
    ...
    plt.subplot(2, 2, 1)
    sns.boxplot(x='label', y='flesch_reading_ease', data=df_sample)
    plt.title('Flesch Reading Ease by Class')
    plt.xlabel('Class (0=Real, 1=Fake)')
    
    plt.subplot(2, 2, 2)
    sns.boxplot(x='label', y='flesch_kincaid_grade', data=df_sample)
    plt.title('Flesch-Kincaid Grade Level by Class')
    plt.xlabel('Class (0=Real, 1=Fake)')
    
    plt.subplot(2, 2, 3)
    sns.boxplot(x='label', y='exclamation_count', data=df_sample)
    plt.title('Exclamation Marks Count by Class')
    plt.xlabel('Class (0=Real, 1=Fake)')
    
    plt.subplot(2, 2, 4)
    sns.boxplot(x='label', y='uppercase_ratio', data=df_sample)
    plt.title('Uppercase Ratio by Class')
    plt.xlabel('Class (0=Real, 1=Fake)')
...
def analyze_clickbait_patterns(df):
    ...
    # Plot clickbait score distribution
    plt.figure(figsize=(10, 6))
